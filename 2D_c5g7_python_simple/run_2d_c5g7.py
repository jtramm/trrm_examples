...
###############################################################################
# Mesh Overlay Subdivision
###############################################################################

# This is the ONLY subdivision knob in this simplified input deck.
#
# Rather than manually carving each pincell into rings and azimuthal
# sectors and hand-building fine/coarse reflector lattices (as the
# 2D_c5g7_python version does), we build the geometry with plain Monte
# Carlo style cells (a single fuel cylinder inside a moderator square)
# and let the random ray solver subdivide each cell on the fly using a
# regular mesh overlaid on the root universe.
#
# `subdivisions_per_pincell` controls how many mesh voxels span a single
# 1.26 cm pincell in each direction. The flat-source random ray solver
# treats each (cell, voxel) intersection as an independent source region,
# so this gives uniform spatial refinement everywhere with one setting.
subdivisions_per_pincell = 7

# The active core is a 3x3 lattice of 21.42 cm assemblies, i.e. 51 pincells
# (3 * 17) across the 64.26 cm domain.
pincells_across = 51
mesh_dim = pincells_across * subdivisions_per_pincell
...
###############################################################################
# Pin Cell Universes (plain Monte Carlo style)
###############################################################################

# A single fuel/absorber cylinder of radius 0.54 cm surrounded by moderator.
# No rings, no azimuthal sectors -- the random ray mesh overlay handles all
# spatial refinement.
pin_radius = 0.54


def make_pincell(name, fuel_material):
    """Build a simple pincell universe: fuel cylinder inside a moderator."""
    cylinder = openmc.ZCylinder(r=pin_radius, name=f'{name} ZCylinder')
    fuel_cell = openmc.Cell(name=name, fill=fuel_material, region=-cylinder)
    mod_cell = openmc.Cell(name=f'{name} Moderator', fill=materials['Water'],
                           region=+cylinder)
    return openmc.Universe(name=name, cells=[fuel_cell, mod_cell])


universes = {}
universes['UO2']             = make_pincell('UO2', materials['UO2'])
universes['MOX 4.3%']        = make_pincell('MOX 4.3%', materials['MOX 4.3%'])
universes['MOX 7.0%']        = make_pincell('MOX 7.0%', materials['MOX 7.0%'])
universes['MOX 8.7%']        = make_pincell('MOX 8.7%', materials['MOX 8.7%'])
universes['Fission Chamber'] = make_pincell('Fission Chamber', materials['Fission Chamber'])
universes['Guide Tube']      = make_pincell('Guide Tube', materials['Guide Tube'])
universes['Control Rod']     = make_pincell('Control Rod', materials['Control Rod'])

# Plain moderator (reflector) universe -- a single water cell that fills its
# lattice element. The mesh overlay subdivides it just like everything else.
universes['Reflector'] = openmc.Universe(
    name='Reflector',
    cells=[openmc.Cell(name='Reflector', fill=materials['Water'])])
...
lattices['UO2 Unrodded Assembly'] = openmc.RectLattice(name='UO2 Unrodded Assembly')
...
lattices['MOX Unrodded Assembly'] = openmc.RectLattice(name='MOX Unrodded Assembly')
...
# Add lattices to cells that bound the 21.42 cm assembly footprint.
cells = {}
...
universes['UO2 Unrodded Assembly'] = openmc.Universe(name='UO2 Unrodded Assembly',
                                                     cells=[cells['UO2 Unrodded Assembly']])
universes['MOX Unrodded Assembly'] = openmc.Universe(name='MOX Unrodded Assembly',
                                                     cells=[cells['MOX Unrodded Assembly']])

###############################################################################
# Core Lattice and Geometry
###############################################################################

# Outer core boundaries (quarter-core symmetry). x-min / y-max are reflective,
# x-max / y-min are vacuum.
core_xmin = openmc.XPlane(x0=-32.13, boundary_type='reflective')
core_xmax = openmc.XPlane(x0=+32.13, boundary_type='vacuum')
core_ymin = openmc.YPlane(y0=-32.13, boundary_type='vacuum')
core_ymax = openmc.YPlane(y0=+32.13, boundary_type='reflective')
core_zmin = openmc.ZPlane(z0=-32.13, boundary_type='reflective')
core_zmax = openmc.ZPlane(z0=+32.13, boundary_type='reflective')
...
uo2 = universes['UO2 Unrodded Assembly']
mox = universes['MOX Unrodded Assembly']
w = universes['Reflector']
lattices['Core'].universes = [[uo2, mox, w],
                              [mox, uo2, w],
                              [w,   w,   w]]

core_region = +core_xmin & -core_xmax & +core_ymin & -core_ymax & \
              +core_zmin & -core_zmax
core_cell = openmc.Cell(name='Core', fill=lattices['Core'], region=core_region)
...
###############################################################################
# Random Ray Mesh Overlay (replaces all manual subdivision)
###############################################################################

divide_mesh = openmc.RegularMesh()
divide_mesh.dimension = (mesh_dim, mesh_dim)
divide_mesh.lower_left = (-32.13, -32.13)
divide_mesh.upper_right = (32.13, 32.13)
settings_file.random_ray['source_region_meshes'] = [(divide_mesh, [root_universe])]
...
# 51x51 pin-resolved fission mesh (matches pin_power_error.py / reference.txt)
